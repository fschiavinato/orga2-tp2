...
// Calculamos el promedio 20%-podado superiormente, o sea nos sacamos los 20% valores mas altos.
...
void correr_exp(enum experimentos exp, int tm, int step, int n, int ldr_alpha, int r, int g, int b, int a) {
    ...
    switch(exp) {
        ...
        case aleatorio_ldr_c: 
            ...
            break;
        case experimentoa_ldr_c: 
            fa = fopen(PATH"experimento_aleatorio_ldr_c_avg.csv", "w");
            fs = fopen(PATH"experimento_aleatorio_ldr_c_sd.csv", "w");
            break;    
        ...
        case constante_ldr_c: 
            ...
            break;
        case experimentoc_ldr_c: 
            fa = fopen(PATH"experimento_constante_ldr_c_avg.csv", "w");
            fs = fopen(PATH"experimento_constante_ldr_c_sd.csv", "w");
            break;   
    ...
    for( i = step; i <= tm; i+= step) {
        for( j = step; j <= tm; j+= step) {
            ...
            switch(exp) {
                ...
                case constante_sepia_asm:
                case constante_sepia_c:
                     img = generador_constante(i, j, r, g, b, a);
                    break;
            }
            imgD = generador_constante(i, j, 0, 0, 0, 0);
            ...
            for( k = 0; k < n; k++) {
                switch(exp) {
                    ...
                    case aleatorio_cropflip_c:
                        ...
                        cropflip_c(bmp_data(img), bmp_data(imgD), j, i, j*4, j*4, j, i, 0, 0);
                        MEDIR_TIEMPO_STOP(mediciones[k]);
                        break;
                    case constante_cropflip_asm: 
                    case aleatorio_cropflip_asm: 
                        MEDIR_TIEMPO_START(start);
                        cropflip_asm(bmp_data(img), bmp_data(imgD), j, i, j*4, j*4, j, i, 0, 0);
                        MEDIR_TIEMPO_STOP(mediciones[k]);
                        break;
                    case constante_ldr_asm:
                    case aleatorio_ldr_asm:
                        MEDIR_TIEMPO_START(start);
                        ldr_asm(bmp_data(img), bmp_data(imgD), j, i, j*4, j*4, ldr_alpha);
                        MEDIR_TIEMPO_STOP(mediciones[k]);
                        break;
                    case constante_ldr_c: 
                    case aleatorio_ldr_c:                     
                        MEDIR_TIEMPO_START(start);
                        ldr_c(bmp_data(img), bmp_data(imgD), j, i, j*4, j*4, ldr_alpha);
                        MEDIR_TIEMPO_STOP(mediciones[k]);
                        break;
                    case experimentoc_ldr_c: 
                    case experimentoa_ldr_c:                     
                    ...
                    case aleatorio_sepia_asm:
                        MEDIR_TIEMPO_START(start);
                        sepia_asm(bmp_data(img), bmp_data(imgD), j, i, j*4, j*4);
                        MEDIR_TIEMPO_STOP(mediciones[k]);
                        break;
                    case constante_sepia_c:
                    case aleatorio_sepia_c:
                        MEDIR_TIEMPO_START(start);
                        sepia_c(bmp_data(img), bmp_data(imgD), j, i, j*4, j*4);
                        MEDIR_TIEMPO_STOP(mediciones[k]);
                        break;
                }
                mediciones[k] -= start;
            ...
            sd = sd / na;
            sd = sqrt(sd);
            fprintf(fa, "%f, ", ((double) avg) / ((double) (i*j)));
            fprintf(fs, "%f, ", ((double) sd) / ((double) (i*j)));
            bmp_delete(img);
            bmp_delete(imgD);
        }
        fprintf(fa, "\n");
